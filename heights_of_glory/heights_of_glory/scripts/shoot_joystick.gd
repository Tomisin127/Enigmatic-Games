...
var shoot_analog_instance_id


func _ready():
	shoot_analog_instance_id = get_instance_id()
		...
		# If the event is a release...
			...
			if (joystick_active == true):
				...
				if event is InputEventScreenTouch:
					...
					#auto shoot if the shoot button is pressed
					emit_signal("auto_attack",true if joystick_touch_id == event_ID and (!global.shoot_drag >0.7) and (global.player_mana_copy > 0) else false)
					global.shoot_drag=0
